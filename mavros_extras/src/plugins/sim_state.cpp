...
private:
    ...
    plugin::filter::SystemAndOk filter [[maybe_unused]])
  {
    ...
    // NOTE: the higher-precision SIM_STATE.lat_int / lon_int (int32, degE7) fields were added
    // upstream after the MAVLink release vendored in this workspace (aerial/mavlink), so they are
    // absent from the generated SIM_STATE struct here -> "no member named 'lat_int'". Use the
    // float lat/lon fields the vendored dialect does provide. Restore the int-field preference when
    // the vendored mavlink is bumped to a release that carries them.
